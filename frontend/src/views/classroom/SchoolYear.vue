<template>
  ...
  export default {
    ...
    computed: {
      ...
      toString() {
        return this.schoolYears.map((schoolYear, index) => {
      ...
      filter() {
        return this.schoolYears.filter((schoolYear, index) => {
      ...
      filtered() {
        if(!this.searchText) {
          this.totalRow = this.schoolYears.length;
      ...
      setPages() {
        ...
        return this.filtered.filter((schoolYear, index) => {
          return(
    ...
    methods: {
      checkCreate(value) {
        ...
        // });
        for(var schoolYear of this.schoolYears) {
          if(schoolYear.name === value) {
            return true;
          }
        }
        return false;
      ...
      async getAll() {
        try {
          this.schoolYears = await SchoolYear.getAll();
        }catch(err) {
          console.log(err);
        }
      },
      ...
      async create() {
        const {value: text} = await Swal.fire({
          ...
          title: 'Add New schoolYear',
          reverseButtons: true,
          input: 'text',
          inputAutoTrim: true,
          inputPlaceholder: 'Enter a new school year',
          text: 'Format: 2021 - 2022',
          inputValidator: (value) => {
            if(!value) {
              return 'You need to write schoolYear.';
            }
            if(this.checkCreate(value)) {
              console.log(this.checkCreate(value));
              console.log(value);
              return 'schoolYear already exists.'
            }
          }
        });
        ...
        if(text) {
          try {
            await SchoolYear.create({name: `${text}`});
            Swal.fire({
              background: this.background,
              color: 'white',
      ...
      async getOne(id) {
        try{
          this.schoolYear = await SchoolYear.get(id);
        }catch(err) {
          console.log(err);
        }
      },
      ...
      async update() {
        const {value: text} = await Swal.fire({
          ...
          input: 'text',
          inputAutoTrim: true,
          inputValue: this.schoolYear.name,
          inputValidator: (value) => {
            if(!value) {
              return 'Data to update can not be empty.';
            }
            if(this.checkCreate(value)) {
              return 'School year already exists.';
            }
          }
        });
        ...
        if(text) {
          try {
            await SchoolYear.update(this.editValue, {name: `${text}`});
            Swal.fire({
              background: this.background,
              color: 'white',
      ...
      async deleteOne() {
        const option = Swal.fire({
          ...
          color: 'white',
          icon: 'warning',
          html: '<p>I want to remove this' + ' \"' + '<span class="text-blue-500 text-lg">' + this.schoolYear.name
            + '</span>' + '\"' + '</p>'
            ,
          showCancelButton: true,
